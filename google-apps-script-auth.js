 ...
 * @returns {Object} 驗證結果
 *
 * ⚠️ 重要：用戶名和密碼必須與 Google Sheets 上的資料「完全一致」才能登入
 * - 用戶名會去除前後空白後進行比對
 * - 密碼是區分大小寫的精確比對
 * - Google Sheets 中的資料也會去除前後空白後比對
 */
function verifyUser(username, password) {
  ...
  const trimmedPassword = password ? password.toString() : ''; // 密碼保留原樣，不去除空白
  ...
  if (!userSheet) {
    ...
    return { success: false, authenticated: false, message: '用戶系統已初始化，請使用 admin / admin123 登入' };
  }
  ...
  if (usernameCol === -1 || passwordCol === -1) {
    return { success: false, authenticated: false, message: '用戶工作表格式錯誤' };
  }
  ...
  for (let i = 1; i < updatedData.length; i++) {
    ...
    // ⚠️ 精確匹配：去除 Google Sheets 中用戶名的前後空白後進行比對
    ...
    if (sheetUsername === trimmedUsername) {
      ...
      if (sheetPassword === trimmedPassword) {
        ...
        // ⚠️ 返回必須包含 authenticated: true 以區別於資料回應
        return { success: true, authenticated: true, message: '登入成功', username: trimmedUsername };
      } else {
        // 密碼錯誤，增加失敗次數
        ...
        return {
          success: false,
          authenticated: false,
          message: '密碼錯誤（剩餘 ' + remaining + ' 次嘗試機會）\n⚠️ 請確認密碼與 Google Sheets 完全一致（區分大小寫）',
  ...
  // 用戶不存在
  return { success: false, authenticated: false, message: '用戶不存在\n⚠️ 請確認用戶名與 Google Sheets「User」工作表中的 Username 欄位完全一致' };
}
